...
On the VPS, ensure `/opt/asoftech` is a git clone of the repo and the user has docker access:

```bash
sudo mkdir -p /opt/asoftech && sudo chown -R asoftech:asoftech /opt/asoftech
sudo usermod -aG docker asoftech
sudo -u asoftech git clone git@github.com:<you>/asoftech-insightz.git /opt/asoftech
# Make sure the VPS deploy user can run docker without sudo and edit /opt/asoftech without sudo
```

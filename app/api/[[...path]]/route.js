import { NextResponse } from 'next/server'
import { v4 as uuid } from 'uuid'
import { getDb } from '@/lib/mongo'
import { aiScore, ruleScore } from '@/lib/scoring'
import { predictShelfLife } from '@/lib/retail-ai'
import { loginUrl, exchangeSessionId, AUTH_CONFIGURED } from '@/lib/auth'
import { resolveTenant, ensureUserOrg, DEMO_ORG_ID } from '@/lib/tenant'
import {
  getRazorpay, RZP_CONFIGURED, PLANS,
  verifyCheckoutSignature, verifyWebhookSignature,
} from '@/lib/razorpay'
import { mobileRoute } from '@/lib/mobile-routes'
import { generateLeadCsv, readExportFile } from '@/lib/csv-export'
import { generateLeadXlsx } from '@/lib/xlsx-export'
import { generateReportPdf } from '@/lib/pdf/report-generator'
import { handleScanner } from '@/lib/scanner/handler'
import { isProductionMode, isWebhookTokenSecure } from '@/lib/security/production'
import { getOrgBranding } from '@/lib/branding/service'
import { applyActiveLeadFilter, softDeleteLead, findLeadById, getLeadDetail, resolveLeadIds } from '@/lib/leads/service'
import { emitPlatformEvent } from '@/lib/events/bus'
import { emitLeadLifecycleEvent } from '@/lib/events/emit-helpers'
import { extractTraceContext } from '@/lib/events/correlation'
import { PLATFORM_EVENTS } from '@/lib/events/types'
import { resolvePlanCode } from '@/lib/billing/plan-map'
import { findCheckoutPlan, productsForPlan } from '@/lib/billing/subscription-plans'
import { getRecentActivities } from '@/lib/activities/recent-activities'
import { listActivities } from '@/lib/activities/service'
import { resolveLeadAttribution } from '@/lib/attribution/enrich-lead'

export const dynamic = 'force-dynamic'

const json = (data, init = {}) => NextResponse.json(data, init)
const err = (m, s = 400) => NextResponse.json({ error: m }, { status: s })

// ─── constants ───
const AGENTS = [
  { id: 'a1', name: 'Aarav Sharma',  territory: 'Bengaluru' },
  { id: 'a2', name: 'Priya Iyer',    territory: 'Bengaluru' },
  { id: 'a3', name: 'Rohan Mehta',   territory: 'Mumbai' },
  { id: 'a4', name: 'Neha Kapoor',   territory: 'Delhi NCR' },
  { id: 'a5', name: 'Vikram Singh',  territory: 'Hyderabad' },
  { id: 'a6', name: 'Anjali Reddy',  territory: 'Chennai' },
  { id: 'a7', name: 'Karthik Nair',  territory: 'Pune' },
]
const STATUSES = ['New', 'Contacted', 'Qualified', 'Proposal', 'Negotiation', 'Won', 'Lost']

function assignAgent(territory) {
  const pool = AGENTS.filter(a => a.territory === territory)
  const list = pool.length ? pool : AGENTS
  return list[Math.floor(Math.random() * list.length)]
}

// ─── one-time demo seeding scoped to DEMO_ORG_ID ───
async function seedDemoLeadsIfEmpty(db) {
  const c = db.collection('leads')
  const n = await c.countDocuments({ orgId: DEMO_ORG_ID })
  if (n > 0) return
  const sources = ['website', 'facebook', 'google', 'whatsapp', 'referral']
  const territories = ['Bengaluru', 'Mumbai', 'Delhi NCR', 'Hyderabad', 'Chennai', 'Pune']
  const samples = [
    { name: 'Rahul Verma',  email: 'rahul@acmepharma.in', phone: '+919812345671', company: 'Acme Pharma',   message: 'Need a demo of LeadEdge360 for our 20 reps. Budget approved.', budget: 250000, whatsapp: true },
    { name: 'Sneha Patel',  email: 'sneha@bharatrealty.com', phone: '+919812345672', company: 'Bharat Realty', message: 'Interested in pricing for territory mapping.', budget: 120000, whatsapp: true },
    { name: 'Aditya Rao',   email: 'a.rao@swiftmart.com',  phone: '+919812345673', company: 'SwiftMart',    message: 'Looking to buy a CRM, please share quote.', budget: 80000,  whatsapp: false },
    { name: 'Meera Joshi',  email: 'meera@velofin.in',     phone: '+919812345674', company: 'VeloFin',      message: 'Just exploring options.', budget: 0,      whatsapp: false },
    { name: 'Karan Bhatia', email: 'karan@greenchem.in',   phone: '+919812345675', company: 'GreenChem',    message: 'Urgent! Need WhatsApp follow-up automation by next week.', budget: 350000, whatsapp: true },
    { name: 'Pooja Shah',   email: 'pooja@novaedu.com',    phone: '+919812345676', company: 'NovaEdu',      message: 'Want a trial.', budget: 30000,  whatsapp: true },
    { name: 'Sahil Khan',   email: 'sahil@trinityauto.in', phone: '+919812345677', company: 'Trinity Auto', message: 'Ready to buy. Decision this week.', budget: 500000, whatsapp: true },
    { name: 'Ishita Sen',   email: 'ishita@lotusclinic.in',phone: '+919812345678', company: 'Lotus Clinic', message: 'Curious how AI scoring works.', budget: 0,      whatsapp: false },
  ]
  const now = Date.now()
  const docs = []
  for (let i = 0; i < samples.length; i++) {
    const s = samples[i]
    const territory = territories[i % territories.length]
    const source = sources[i % sources.length]
    const lead = { ...s, source, territory }
    const sc = await aiScore(lead)
    const agent = assignAgent(territory)
    docs.push({
      id: uuid(), orgId: DEMO_ORG_ID,
      ...lead,
      score: sc.score, label: sc.label, reasons: sc.reasons, engine: sc.engine,
      status: STATUSES[i % 6],
      assignedTo: agent.name, assignedAgentId: agent.id,
      createdAt: new Date(now - (samples.length - i) * 86400000 * 1.5).toISOString(),
      updatedAt: new Date().toISOString(),
    })
  }
  await c.insertMany(docs)
}

async function seedDemoProductsIfEmpty(db) {
  const c = db.collection('products')
  const n = await c.countDocuments({ orgId: DEMO_ORG_ID })
  if (n > 0) return
  const samples = [
    { name: 'Amul Gold Milk 1L',          sku: 'DAIRY-001', category: 'dairy',     price: 70,   stock: 240, daysOnShelf: 3,  expiryDate: addDays(5),  store: 'Bengaluru-01' },
    { name: 'Britannia Brown Bread',       sku: 'BAK-014',   category: 'bakery',    price: 45,   stock: 110, daysOnShelf: 2,  expiryDate: addDays(3),  store: 'Bengaluru-01' },
    { name: 'Tropicana Orange Juice 1L',   sku: 'BEV-202',   category: 'beverage',  price: 120,  stock: 80,  daysOnShelf: 12, expiryDate: addDays(60), store: 'Mumbai-02' },
    { name: 'Crocin Advance 500mg 15s',    sku: 'PHR-330',   category: 'pharma',    price: 35,   stock: 320, daysOnShelf: 90, expiryDate: addDays(240),store: 'Delhi-03' },
    { name: 'Lakme 9to5 Lipstick Ruby',    sku: 'COSM-501',  category: 'cosmetic',  price: 540,  stock: 45,  daysOnShelf: 180,expiryDate: addDays(400),store: 'Delhi-03' },
    { name: 'Chicken Breast 500g',         sku: 'MEAT-09',   category: 'meat',      price: 280,  stock: 25,  daysOnShelf: 2,  expiryDate: addDays(2),  store: 'Hyderabad-04' },
    { name: 'Surf Excel Matic Top 1kg',    sku: 'HHD-77',    category: 'household', price: 230,  stock: 160, daysOnShelf: 20, expiryDate: addDays(540),store: 'Chennai-05' },
    { name: 'JBL Tune 110 Earphones',      sku: 'ELEC-12',   category: 'electronics',price: 999, stock: 30,  daysOnShelf: 60, expiryDate: addDays(1500),store: 'Bengaluru-01' },
    { name: 'Bananas Robusta 1kg',          sku: 'PROD-04',   category: 'produce',   price: 60,   stock: 95,  daysOnShelf: 1,  expiryDate: addDays(3),  store: 'Pune-06' },
  ]
  // Parallelize the LLM predictions to keep first-load snappy.
  const preds = await Promise.all(samples.map(s => predictShelfLife(s)))
  const docs = samples.map((s, i) => ({
    id: uuid(), orgId: DEMO_ORG_ID, ...s, ...preds[i],
    createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true, updatedAt: new Date().toISOString(),
  }))
  await c.insertMany(docs)
}

// One-time migration: ensure every doc without orgId is tagged demo-org so the
// pre-tenant data still shows up in the demo dashboard.
let migrationDone = false
async function ensureLegacyMigration(db) {
  if (migrationDone) return
  migrationDone = true
  await db.collection('leads').updateMany({ orgId: { $exists: false } }, { $set: { orgId: DEMO_ORG_ID } })
  await db.collection('products').updateMany({ orgId: { $exists: false } }, { $set: { orgId: DEMO_ORG_ID } })
}
function addDays(d) { const x = new Date(); x.setDate(x.getDate() + d); return x.toISOString() }

// ─── webhook helper ───
function ingestWebhookAllowed(req) {
  if (!isWebhookTokenSecure()) return process.env.NODE_ENV !== 'production'
  return req.headers.get('x-webhook-token') === process.env.N8N_WEBHOOK_TOKEN
}

// ─── master router ───
async function route(method, segs, request) {
  const [root, id, action] = segs

  // public health
  if (!root) return json({ ok: true, name: 'AsoftechInsightz API', time: new Date().toISOString() })

  const { tryDevLoginPassword, tryDevApiRoute, isDevAuthBypassEnabled, buildDevTokenResponse } = await import('@/lib/dev-auth')

  // ─────────────────────────  AUTH  ─────────────────────────
  if (root === 'auth') {
    let authRequest = request

    if (id === 'login-password' && method === 'POST') {
      const body = await request.json()
      const dev = tryDevLoginPassword(body?.email, body?.password)
      if (dev) return json(dev)
      authRequest = new Request(request.url, {
        method: request.method,
        headers: request.headers,
        body: JSON.stringify(body),
      })
    }
    if (id === 'refresh-token' && method === 'POST' && isDevAuthBypassEnabled()) {
      const body = await request.json().catch(() => ({}))
      if (String(body.refreshToken || '').startsWith('dev-refresh-')) {
        return json(buildDevTokenResponse())
      }
      authRequest = new Request(request.url, {
        method: request.method,
        headers: request.headers,
        body: JSON.stringify(body),
      })
    }
    if (id === 'me' && method === 'GET') {
      const { user, isDemo } = await resolveTenant(request)
      return json({ user, isDemo, configured: AUTH_CONFIGURED })
    }
    if (id === 'logout' && method === 'POST') {
      const res = json({ ok: true })
      res.cookies.delete('emergent_session')
      return res
    }

    let db
    try {
      db = await getDb()
    } catch (mongoErr) {
      if (id === 'login-password' && method === 'POST') {
        return NextResponse.json(
          { code: 'AUTH_INVALID_CREDENTIALS', message: 'Email or password is incorrect' },
          { status: 401 }
        )
      }
      throw mongoErr
    }
    // GET /api/auth/login → redirect to Emergent hosted login
    if (id === 'login' && method === 'GET') {
      if (!AUTH_CONFIGURED) {
        // Graceful fallback when keys aren't set yet
        return NextResponse.redirect(new URL('/signin?auth_error=not_configured', process.env.NEXT_PUBLIC_APP_URL))
      }
      const redirectUri = `${process.env.NEXT_PUBLIC_APP_URL || new URL(request.url).origin}/api/auth/callback`
      return NextResponse.redirect(loginUrl(redirectUri))
    }
    // GET /api/auth/callback?session_id=...
    if (id === 'callback' && method === 'GET') {
      const sessionId = new URL(request.url).searchParams.get('session_id')
      if (!sessionId) return NextResponse.redirect(new URL('/?auth_error=missing_session', process.env.NEXT_PUBLIC_APP_URL))
      const data = await exchangeSessionId(sessionId)
      if (!data) return NextResponse.redirect(new URL('/?auth_error=exchange_failed', process.env.NEXT_PUBLIC_APP_URL))
      await ensureUserOrg(data.user)
      const res = NextResponse.redirect(new URL('/dashboard', process.env.NEXT_PUBLIC_APP_URL))
      res.cookies.set({
        name: 'emergent_session', value: data.session_token,
        httpOnly: true, secure: process.env.NODE_ENV === 'production',
        sameSite: 'lax', path: '/', maxAge: 60 * 60 * 24 * 7,
      })
      return res
    }
    // POST /api/auth/dpdp-consent  — record DPDP Act consent
    if (id === 'dpdp-consent' && method === 'POST') {
      const { user } = await resolveTenant(request)
      if (!user) return err('Not signed in', 401)
      const body = await request.json()
      const consent = {
        accepted: true,
        acceptedAt: new Date().toISOString(),
        version: '1.0',
        ip: request.headers.get('x-forwarded-for') || '',
        ua: request.headers.get('user-agent') || '',
        ...body,
      }
      await db.collection('users').updateOne({ email: user.email }, { $set: { dpdpConsent: consent } })
      await db.collection('consent_log').insertOne({ id: uuid(), email: user.email, ...consent })
      return json({ ok: true, consent })
    }
    // Fall through to mobileRoute for /auth/register, /auth/verify-otp, /auth/login-password, etc.
    const mAuth = await mobileRoute({ root, id, action, segs, method, request: authRequest, db })
    if (mAuth) return mAuth
    return err('Not found', 404)
  }

  if (isDevAuthBypassEnabled()) {
    const devRes = await tryDevApiRoute(method, segs, request)
    if (devRes) return devRes
  }

  const db = await getDb()

  // From here, every other route resolves the tenant first.
  const tenant = await resolveTenant(request)
  const { orgId } = tenant

  const PUBLIC_ROOTS = new Set(['auth', 'health', 'webhooks', 'scanner'])
  if (isProductionMode() && !PUBLIC_ROOTS.has(root) && !tenant.user) {
    return NextResponse.json({ success: false, error: 'UNAUTHORIZED' }, { status: 401 })
  }

  // Demo seed runs via getDb() in development

  // ─── Mobile API routes (JWT-authenticated) ───
  // Tries auth/register, /auth/verify-otp, /auth/login-password, /auth/refresh-token,
  // /users/*, /followups/*, /dashboard/*, /whatsapp/*, /notifications/*, /admin/*.
  // Returns null when no match → falls through to existing handlers below.
  const mobile = await mobileRoute({ root, id, action, segs, method, request, db })

  if (
    root === 'mobile' &&
    (id === 'home' || id === 'summary')
  ) {
    // handled locally below
  } else   if (mobile) {
    return mobile
  }

  // ─────────────────────────  INTEGRATIONS  ─────────────────────────
  if (root === 'integrations') {
    const { handleIntegrationsCatchall } = await import('@/lib/integrations/catchall-handler')
    const intRes = await handleIntegrationsCatchall({ method, segs, request, tenant })
    if (intRes) return intRes
    return err('Not found', 404)
  }

  // ─────────────────────────  AGENTS  ─────────────────────────
  if (root === 'agents' && method === 'GET') return json({ agents: AGENTS })

  // ─────────────────────────  SCANNER  ─────────────────────────
  if (root === 'scanner') {

    const response = await handleScanner({
      method,
      id,
      request,
      orgId,
      user: tenant.user
    })

    if (response) {
      return json(response)
    }
  }

  // ─────────────────────────  LEADS  ─────────────────────────
  if (root === 'leads') {
    const col = db.collection('leads')

    // GET /leads/sources → distinct sources for this tenant
    if (method === 'GET' && id === 'sources') {
      const list = await col.distinct('source', { orgId })
      return json({ sources: list.map(code => ({ code, name: code.charAt(0).toUpperCase() + code.slice(1) })) })
    }

    if (method === 'GET' && !id) {
      const url = new URL(request.url)
      const territory = url.searchParams.get('territory')
      const status = url.searchParams.get('status')
      const label = url.searchParams.get('label')
      const source = url.searchParams.get('source')
      const role = url.searchParams.get('role')
      const agentName = url.searchParams.get('agent')
      const assignedTo = url.searchParams.get('assignedTo')
      const search = url.searchParams.get('q')
      const from = url.searchParams.get('createdFrom')
      const to = url.searchParams.get('createdTo')
      const q = applyActiveLeadFilter({ orgId })
      if (territory && territory !== 'all') q.territory = territory
      if (status && status !== 'all') q.status = status
      if (label && label !== 'all') q.label = label
      if (source && source !== 'all') q.source = source.toLowerCase()
      if (role === 'agent' && agentName) q.assignedTo = agentName
      if (assignedTo) q.assignedTo = assignedTo
      if (from || to) q.createdAt = { ...(from ? { $gte: from } : {}), ...(to ? { $lte: to } : {}) }
      if (search) {
        const rx = new RegExp(search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')
        q.$or = [{ name: rx }, { email: rx }, { phone: rx }, { company: rx }]
      }
      const sortRaw = url.searchParams.get('sort') || '-createdAt'
      const sortField = sortRaw.replace(/^-/, '')
      const sort = { [sortField]: sortRaw.startsWith('-') ? -1 : 1 }
      const page = Math.max(1, parseInt(url.searchParams.get('page') || '1'))
      const pageSize = Math.min(200, parseInt(url.searchParams.get('pageSize') || '50'))
      const total = await col.countDocuments(q)
      const list = await col.find(q, { projection: { _id: 0 } }).sort(sort).skip((page - 1) * pageSize).limit(pageSize).toArray()
      return json({ leads: list, meta: { page, pageSize, total, hasMore: page * pageSize < total } })
    }
    if (method === 'POST' && !id) {
      const body = await request.json()
      if (!body?.name || !body?.phone) return err('name and phone are required')
      const territory = body.territory || 'Bengaluru'
      const lead = {
        name: body.name, email: body.email || '', phone: body.phone,
        company: body.company || '', message: body.message || '',
        budget: Number(body.budget || 0), whatsapp: !!body.whatsapp,
        source: (body.source || 'website').toLowerCase(), territory,
      }
      // Fast rule-based score on create; LLM rescore via POST /leads/:id/rescore or agent runtime.
      const sc = ruleScore(lead)
      const agent = assignAgent(territory)
      const doc = {
        id: uuid(), orgId, ...lead,
        score: sc.score, label: sc.label, reasons: sc.reasons, engine: sc.engine,
        status: 'New',
        assignedTo: agent.name, assignedAgentId: agent.id,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      }
      await col.insertOne({ ...doc })
      const trace = extractTraceContext(request)
      void emitPlatformEvent({
        db,
        orgId,
        type: PLATFORM_EVENTS.LEAD_CREATED,
        entity: 'lead',
        entityId: doc.id,
        payload: { source: doc.source, name: doc.name, score: doc.score, leadId: doc.id },
        userId: tenant.user?.id || null,
        source: 'api',
        ...trace,
      }).catch((e) => console.error('[leads] lead.created event failed:', e.message))
      return json({ lead: doc }, { status: 201 })
    }
    if (method === 'PATCH' && id && !action) {
      const body = await request.json()
      const existing = await col.findOne({ id, orgId }, { projection: { _id: 0 } })
      if (!existing) return err('Lead not found', 404)
      const update = { updatedAt: new Date().toISOString() }
      const now = new Date()
      if (body.name) update.name = body.name
      if (body.email !== undefined) update.email = body.email
      if (body.phone) update.phone = body.phone
      if (body.company !== undefined) update.company = body.company
      if (body.territory !== undefined) update.territory = body.territory
      if (body.message !== undefined) update.message = body.message
      if (body.budget !== undefined) update.budget = Number(body.budget || 0)
      if (body.status && STATUSES.includes(body.status)) {
        update.status = body.status
        if (body.status !== existing.status) {
          await db.collection('lead_timeline').insertOne({
            id: uuid(), orgId, leadId: id, type: 'status_change',
            payload: { from: existing.status, to: body.status, reason: body.reason || 'Updated via PATCH' },
            createdAt: now,
          })
          await db.collection('lead_status_history').insertOne({
            id: uuid(),
            orgId,
            leadId: id,
            fromStatus: existing.status,
            toStatus: body.status,
            reason: body.reason || 'Updated via PATCH',
            createdAt: now,
          })
        }
      }
      if (body.assignedTo) {
        const ag = AGENTS.find(a => a.name === body.assignedTo)
        update.assignedTo = body.assignedTo
        if (ag) update.assignedAgentId = ag.id
        if (body.assignedTo !== existing.assignedTo) {
          await db.collection('lead_timeline').insertOne({
            id: uuid(), orgId, leadId: id, type: 'assigned',
            payload: { to: body.assignedTo }, createdAt: now,
          })
          await db.collection('lead_assignments').insertOne({
            id: uuid(),
            orgId,
            leadId: id,
            assignedTo: body.assignedTo,
            assignedBy: tenant.user?.fullName || tenant.user?.name || tenant.user?.email || 'Admin',
            assignedAgentId: ag?.id || null,
            createdAt: now,
          })
        }
      }
      const r = await col.findOneAndUpdate({ id, orgId }, { $set: update }, { returnDocument: 'after', projection: { _id: 0 } })
      const updated = r?.value || r
      if (!updated) return err('Lead not found', 404)

      const trace = extractTraceContext(request)
      const userId = tenant.user?.id || null
      const changedFields = Object.keys(update).filter((k) => k !== 'updatedAt')

      if (changedFields.length) {
        await emitLeadLifecycleEvent(db, {
          orgId,
          type: PLATFORM_EVENTS.LEAD_UPDATED,
          leadId: id,
          payload: { fields: changedFields, from: existing, to: updated },
          userId,
          source: 'api',
          ...trace,
        })
      }

      if (body.assignedTo && body.assignedTo !== existing.assignedTo) {
        await emitLeadLifecycleEvent(db, {
          orgId,
          type: PLATFORM_EVENTS.LEAD_ASSIGNED,
          leadId: id,
          payload: { from: existing.assignedTo, to: body.assignedTo },
          userId,
          source: 'api',
          ...trace,
        })
      }

      if (body.status && body.status !== existing.status) {
        if (body.status === 'Qualified') {
          await emitLeadLifecycleEvent(db, {
            orgId,
            type: PLATFORM_EVENTS.LEAD_QUALIFIED,
            leadId: id,
            payload: { from: existing.status, to: body.status, score: updated.score },
            userId,
            source: 'api',
            ...trace,
          })
        }
        if (['Won', 'Proposal'].includes(body.status)) {
          await emitLeadLifecycleEvent(db, {
            orgId,
            type: PLATFORM_EVENTS.LEAD_CONVERTED,
            leadId: id,
            payload: { from: existing.status, to: body.status },
            userId,
            source: 'api',
            ...trace,
          })
        }
        if (body.status === 'Lost') {
          await emitLeadLifecycleEvent(db, {
            orgId,
            type: PLATFORM_EVENTS.LEAD_LOST,
            leadId: id,
            payload: { from: existing.status, to: body.status, reason: body.reason },
            userId,
            source: 'api',
            ...trace,
          })
        }
      }

      return json({ lead: updated })
    }
    if (method === 'DELETE' && id) {
      const userId = tenant.user?.id || tenant.user?.email || 'system'
      await softDeleteLead(db, orgId, userId, id, {
        ip: request.headers.get('x-forwarded-for') || '',
        ua: request.headers.get('user-agent') || '',
      })
      return json({ ok: true })
    }
    if (method === 'POST' && id && action === 'rescore') {
      const lead = await col.findOne({ id, orgId }, { projection: { _id: 0 } })
      if (!lead) return err('Lead not found', 404)
      const sc = await aiScore(lead)
      const r = await col.findOneAndUpdate({ id, orgId },
        { $set: { score: sc.score, label: sc.label, reasons: sc.reasons, engine: sc.engine, updatedAt: new Date().toISOString() } },
        { returnDocument: 'after', projection: { _id: 0 } })
      return json({ lead: r?.value || r })
    }

    // GET /leads/:id → details with activities + follow-ups (delegates to shared service when possible)
    if (method === 'GET' && id && !action) {
      try {
        const detail = await getLeadDetail(db, orgId, id)
        return json({ success: true, ...detail, lead: detail.lead })
      } catch (e) {
        if (e.message === 'NOT_FOUND') {
          console.warn('[leads] catch-all GET not found', { leadId: id, orgId })
          return err('Lead not found', 404)
        }
        throw e
      }
    }

    // POST /leads/:id/status
    // GET /leads/:id/notes
    if (method === 'GET' && id && action === 'notes') {
      const leadIds = await resolveLeadIds(db, orgId, id)
      if (!leadIds) return err('Lead not found', 404)
      const rows = await db.collection('lead_notes')
        .find({ orgId, leadId: { $in: leadIds } }, { projection: { _id: 0 } })
        .sort({ createdAt: -1 })
        .toArray()

      return json({ notes: rows })
    }

    // POST /leads/:id/notes
    if (method === 'POST' && id && action === 'notes') {
      const body = await request.json()

      const note = {
        id: uuid(),
        orgId,
        leadId: id,
        note: body.note || '',
        createdAt: new Date().toISOString(),
      }

      await db.collection('lead_notes').insertOne(note)

      await db.collection('lead_timeline').insertOne({
        id: uuid(),
        orgId,
        leadId: id,
        type: 'note_added',
        payload: { note: note.note },
        actorType: 'user',
        actorName: tenant.user?.fullName || tenant.user?.name || tenant.user?.email || 'Team member',
        createdAt: note.createdAt,
      })

      delete note._id

      return json({ note }, { status: 201 })
    }

    // GET /leads/:id/timeline
    if (method === 'GET' && id && action === 'timeline') {
      const leadIds = await resolveLeadIds(db, orgId, id)
      if (!leadIds) return err('Lead not found', 404)
      const rows = await db.collection('lead_timeline')
        .find({ orgId, leadId: { $in: leadIds } }, { projection: { _id: 0 } })
        .sort({ createdAt: -1 })
        .toArray()

      return json({ timeline: rows })
    }

    // POST /leads/:id/timeline
    if (method === 'POST' && id && action === 'timeline') {
      const body = await request.json()

      const doc = {
        id: uuid(),
        orgId,
        leadId: id,
        type: body.type || 'note',
        title: body.title || '',
        description: body.description || '',
        createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
      }

      await db.collection('lead_timeline').insertOne(doc)

      delete doc._id

      return json({ timeline: doc }, { status: 201 })
    }

    if (method === 'POST' && id && action === 'status') {
      const body = await request.json()
      if (!body.status || !STATUSES.includes(body.status))
        return err('Invalid status', 400)
      const existing = await col.findOne({ id, orgId }, { projection: { _id: 0, status: 1 } })
      if (!existing) return err('Lead not found', 404)
      const r = await col.findOneAndUpdate({ id, orgId },
        { $set: { status: body.status, updatedAt: new Date().toISOString() } },
        { returnDocument: 'after', projection: { _id: 0 } })
      const now = new Date()
      await db.collection('lead_timeline').insertOne({
        id: uuid(), orgId, leadId: id, type: 'status_change',
        payload: { from: existing.status, to: body.status, reason: body.reason || '' },
        createdAt: now,
      })
      await db.collection('lead_status_history').insertOne({
        id: uuid(),
        orgId,
        leadId: id,
        fromStatus: existing.status,
        toStatus: body.status,
        reason: body.reason || '',
        createdAt: now,
      })
      return json({ lead: r?.value || r })
    }

    // POST /leads/:id/assign
    if (method === 'POST' && id && action === 'assign') {
      const body = await request.json()
      let assignedTo = body.assignedTo
      if (body.userId && !assignedTo) {
        const u = await db.collection('users').findOne({ id: body.userId, orgId })
        if (u) assignedTo = u.fullName || u.email
      }
      if (!assignedTo) return err('assignedTo or userId required', 400)
      const ag = AGENTS.find(a => a.name === assignedTo)
      const r = await col.findOneAndUpdate({ id, orgId },
        { $set: { assignedTo, assignedAgentId: ag?.id, updatedAt: new Date().toISOString() } },
        { returnDocument: 'after', projection: { _id: 0 } })
      if (!r?.value && !r?.id) return err('Lead not found', 404)
      const now = new Date()
      await db.collection('lead_timeline').insertOne({
        id: uuid(), orgId, leadId: id, type: 'assigned',
        payload: { to: assignedTo }, createdAt: now,
      })
      const assignedBy = tenant.user?.fullName || tenant.user?.name || tenant.user?.email || 'Admin'
      await db.collection('lead_assignments').insertOne({
        id: uuid(),
        orgId,
        leadId: id,
        assignedTo,
        assignedBy,
        assignedAgentId: ag?.id || null,
        createdAt: now,
      })
      return json({ lead: r?.value || r })
    }

    // GET /leads/:id/followups
    if (method === 'GET' && id && action === 'followups') {
      const leadIds = await resolveLeadIds(db, orgId, id)
      if (!leadIds) return err('Lead not found', 404)
      const rows = await db.collection('follow_ups')
        .find({ orgId, leadId: { $in: leadIds } }, { projection: { _id: 0 } })
        .sort({ dueAt: 1 })
        .toArray()

      return json({ followups: rows })
    }

    // POST /leads/:id/followups
    if (method === 'POST' && id && action === 'followups') {
      const body = await request.json()

      const followup = {
        id: uuid(),
        orgId,
        leadId: id,
        title: body.title || '',
        dueAt: body.dueAt,
        status: 'pending',
        createdAt: new Date().toISOString(),
      }

      await db.collection('follow_ups').insertOne(followup)

      const trace = extractTraceContext(request)
      await emitPlatformEvent({
        db,
        orgId,
        type: PLATFORM_EVENTS.FOLLOWUP_CREATED,
        entity: 'lead',
        entityId: id,
        payload: { leadId: id, title: followup.title, dueAt: followup.dueAt },
        userId: tenant.user?.id || null,
        source: 'api',
        ...trace,
      })

      if (/meeting|call|demo|visit/i.test(followup.title || '')) {
        await emitPlatformEvent({
          db,
          orgId,
          type: PLATFORM_EVENTS.MEETING_SCHEDULED,
          entity: 'lead',
          entityId: id,
          payload: { leadId: id, title: followup.title, scheduledAt: followup.dueAt, followupId: followup.id },
          userId: tenant.user?.id || null,
          source: 'api',
          ...trace,
        })
      }

      await db.collection('lead_timeline').insertOne({
        id: uuid(),
        orgId,
        leadId: id,
        type: 'followup_created',
        payload: {
          title: followup.title,
          dueAt: followup.dueAt,
        },
        actorType: 'user',
        actorName: tenant.user?.fullName || tenant.user?.name || tenant.user?.email || 'Team member',
        createdAt: new Date().toISOString(),
      })

      delete followup._id

      return json({ followup }, { status: 201 })
    }

    // POST /leads/:id/tasks
    if (method === 'POST' && id && action === 'tasks') {

      const body = await request.json()

      const task = {
        id: uuid(),
        orgId,
        leadId: id,
        title: body.title || '',
        description: body.description || '',
        dueAt: body.dueAt || null,
        priority: body.priority || 'medium',
        status: 'open',
        createdAt: new Date().toISOString(),
      }

      await db.collection('lead_tasks').insertOne(task)

      const trace = extractTraceContext(request)
      await emitPlatformEvent({
        db,
        orgId,
        type: PLATFORM_EVENTS.TASK_CREATED,
        entity: 'lead',
        entityId: id,
        payload: { leadId: id, title: task.title },
        userId: tenant.user?.id || null,
        source: 'api',
        ...trace,
      })

      await db.collection('lead_timeline').insertOne({
        id: uuid(),
        orgId,
        leadId: id,
        type: 'task_created',
        payload: {
          title: task.title,
          dueAt: task.dueAt,
        },
        actorType: 'user',
        actorName: tenant.user?.fullName || tenant.user?.name || tenant.user?.email || 'Team member',
        createdAt: new Date().toISOString(),
      })

      delete task._id

      return json({ task }, { status:201 })
    }

    // GET /leads/:id/tasks
    if (method === 'GET' && id && action === 'tasks') {
      const rows = await db.collection('lead_tasks')
        .find({ orgId, leadId:id }, { projection:{ _id:0 } })
        .sort({ dueAt:1 })
        .toArray()

      return json({ tasks: rows })
    }    // POST /leads/:id/status

    // GET /leads/:id/status-history
    if (method === 'GET' && id && action === 'status-history') {

      const rows = await db.collection('lead_status_history')
        .find({ orgId, leadId:id }, { projection:{ _id:0 } })
        .sort({ createdAt:-1 })
        .toArray()

      return json({ history: rows })
    }







    // GET /leads/:id/assignments
    if (method === 'GET' && id && action === 'assignments') {
      const leadIds = await resolveLeadIds(db, orgId, id)
      if (!leadIds) return err('Lead not found', 404)
      const rows = await db.collection('lead_assignments')
        .find({ orgId, leadId: { $in: leadIds } }, { projection: { _id: 0 } })
        .sort({ createdAt: -1 })
        .toArray()

      return json({ assignments: rows })
    }

    // GET /leads/:id/activity
    if (method === 'GET' && id && action === 'activity') {
      const lead = await col.findOne({ orgId, id }, { projection: { _id: 0 } })

      if (!lead) return err('Lead not found', 404)

      const [
        notesCount,
        tasksCount,
        openTasks,
        followUpsCount,
        statusChanges,
        assignmentsCount
      ] = await Promise.all([
        db.collection('lead_notes').countDocuments({ orgId, leadId: id }),
        db.collection('lead_tasks').countDocuments({ orgId, leadId: id }),
        db.collection('lead_tasks').countDocuments({
          orgId,
          leadId: id,
          status: { $ne: 'completed' }
        }),
        db.collection('follow_ups').countDocuments({ orgId, leadId: id }),
        db.collection('lead_status_history').countDocuments({ orgId, leadId: id }),
        db.collection('lead_assignments').countDocuments({ orgId, leadId: id })
      ])

      return json({
        leadId: id,
        currentStatus: lead.status || null,
        notesCount,
        tasksCount,
        openTasks,
        followUpsCount,
        statusChanges,
        assignmentsCount
      })
    }

  }



  // GET /lead-dashboard
  if (root === 'lead-dashboard' && method === 'GET') {

    const [
      totalLeads,
      totalTasks,
      openTasks,
      totalFollowUps,
      totalNotes,
      totalStatusChanges
    ] = await Promise.all([
      db.collection('leads').countDocuments({ orgId }),
      db.collection('lead_tasks').countDocuments({ orgId }),
      db.collection('lead_tasks').countDocuments({
        orgId,
        status: { $ne: 'completed' }
      }),
      db.collection('follow_ups').countDocuments({ orgId }),
      db.collection('lead_notes').countDocuments({ orgId }),
      db.collection('lead_status_history').countDocuments({ orgId })
    ])

    return json({
      totalLeads,
      totalTasks,
      openTasks,
      totalFollowUps,
      totalNotes,
      totalStatusChanges
    })
  }


  // GET /followup-dashboard
  if (root === 'followup-dashboard' && method === 'GET') {

    const [
      totalFollowUps,
      pendingFollowUps,
      completedFollowUps
    ] = await Promise.all([
      db.collection('follow_ups').countDocuments({ orgId }),
      db.collection('follow_ups').countDocuments({
        orgId,
        status: 'pending',
      exportType: body.exportType || 'csv',
      format: 'csv'
      }),
      db.collection('follow_ups').countDocuments({
        orgId,
        status: 'completed'
      })
    ])

    return json({
      totalFollowUps,
      pendingFollowUps,
      completedFollowUps
    })
  }


  // GET /task-dashboard
  if (root === 'task-dashboard' && method === 'GET') {

    const [
      totalTasks,
      completedTasks,
      openTasks
    ] = await Promise.all([
      db.collection('lead_tasks').countDocuments({ orgId }),
      db.collection('lead_tasks').countDocuments({
        orgId,
        status: 'completed'
      }),
      db.collection('lead_tasks').countDocuments({
        orgId,
        status: { $ne: 'completed' }
      })
    ])

    return json({
      totalTasks,
      completedTasks,
      openTasks
    })
  }


  // GET /lead-search?q=
  if (root === 'lead-search' && method === 'GET') {

    const url = new URL(request.url)
    const q = (url.searchParams.get('q') || '').trim()

    if (!q) {
      return json({ leads: [] })
    }

    const leads = await db.collection('leads')
      .find(applyActiveLeadFilter({
        orgId,
        $or: [
          { name: { $regex: q, $options: 'i' } },
          { company: { $regex: q, $options: 'i' } },
          { email: { $regex: q, $options: 'i' } },
          { phone: { $regex: q, $options: 'i' } },
          { territory: { $regex: q, $options: 'i' } },
        ],
      }))
      .project({ _id: 0 })
      .limit(25)
      .toArray()

    return json({
      count: leads.length,
      leads
    })
  }


  // GET /lead-filters
  if (root === 'lead-filters' && method === 'GET') {

    const url = new URL(request.url)

    const status = url.searchParams.get('status')
    const assignedTo = url.searchParams.get('assignedTo')
    const source = url.searchParams.get('source')

    const query = { orgId }

    if (status) query.status = status
    if (assignedTo) query.assignedTo = assignedTo
    if (source) query.source = source

    const leads = await db.collection('leads')
      .find(query)
      .project({ _id: 0 })
      .sort({ updatedAt: -1 })
      .toArray()

    return json({
      count: leads.length,
      leads
    })
  }


  // GET /recent-activities (legacy — use /activities)
  if (root === 'recent-activities' && method === 'GET') {
    const result = await getRecentActivities(db, orgId, { limit: 20 })
    return json(result)
  }

  // GET /activities — unified feed with filters, search, pagination
  if (root === 'activities' && method === 'GET') {
    const url = new URL(request.url)
    const result = await listActivities(db, orgId, {
      limit: url.searchParams.get('limit'),
      cursor: url.searchParams.get('cursor'),
      category: url.searchParams.get('filter') || url.searchParams.get('category') || 'all',
      search: url.searchParams.get('search') || url.searchParams.get('q') || '',
      userId: url.searchParams.get('mine') === 'true' ? (tenant.user?.id || tenant.user?.email) : null,
    })
    return json(result)
  }


  // GET /mobile-leads?page=1&limit=20
  if (root === 'mobile-leads' && method === 'GET') {

    const url = new URL(request.url)

    const page = Math.max(
      parseInt(url.searchParams.get('page') || '1'),
      1
    )

    const limit = Math.min(
      parseInt(url.searchParams.get('limit') || '20'),
      100
    )

    const skip = (page - 1) * limit

    const total = await db.collection('leads')
      .countDocuments({ orgId })

    const leads = await db.collection('leads')
      .find({ orgId })
      .project({
        _id: 0,
        id: 1,
        name: 1,
        phone: 1,
        email: 1,
        status: 1,
        assignedTo: 1,
        updatedAt: 1
      })
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limit)
      .toArray()

    return json({
      page,
      limit,
      total,
      totalPages: Math.ceil(total / limit),
      leads
    })
  }


  // GET /mobile/home
  if (root === 'mobile' && id === 'home' && method === 'GET') {

    const [
      totalLeads,
      openTasks,
      pendingFollowUps,
      recentActivitiesResult,
      recentLeads
    ] = await Promise.all([
      db.collection('leads').countDocuments({ orgId }),
      db.collection('lead_tasks').countDocuments({
        orgId,
        status: { $ne: 'completed' }
      }),
      db.collection('follow_ups').countDocuments({
        orgId,
        status: 'pending',
      }),
      getRecentActivities(db, orgId, { limit: 10 }),
      db.collection('leads')
        .find({ orgId })
        .project({ _id:0 })
        .sort({ updatedAt:-1 })
        .limit(10)
        .toArray()
    ])

    return json({
      stats:{
        totalLeads,
        openTasks,
        pendingFollowUps
      },
      recentActivities: recentActivitiesResult.activities,
      recentLeads
    })
  }
  // POST /mobile/leads/:id/note
  if (
    root === 'mobile' &&
    id === 'leads' &&
    action &&
    segs[3] === 'note' &&
    method === 'POST'
  ) {

    const body = await request.json()

    const doc = {
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      note: body.note || '',
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    }

    await db.collection('lead_notes').insertOne(doc);

    await db.collection('audit_logs').insertOne({
      id: crypto.randomUUID(),
      orgId,
      entity: 'lead_note',
      entityId: action,
      action: 'create',
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    await db.collection('lead_timeline').insertOne({
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      type: 'note_added',
      payload: {
        note: body.note || ''
      },
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    return json({
      success: true,
      note: doc
    })
  }


  // POST /mobile/leads/:id/task
  if (
    root === 'mobile' &&
    id === 'leads' &&
    action &&
    segs[3] === 'task' &&
    method === 'POST'
  ) {

    const body = await request.json()

    const doc = {
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      title: body.title,
      description: body.description || '',
      status: 'pending',
      exportType: body.exportType || 'csv',
      format: 'csv',
      dueAt: body.dueAt || null,
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    }

    await db.collection('lead_tasks').insertOne(doc);

    await db.collection('audit_logs').insertOne({
      id: crypto.randomUUID(),
      orgId,
      entity: 'lead_task',
      entityId: action,
      action: 'create',
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    await db.collection('lead_timeline').insertOne({
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      type: 'task_created',
      payload: {
        title: body.title
      },
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    return json({
      success: true,
      task: doc
    })
  }


  // POST /mobile/leads/:id/followup
  if (
    root === 'mobile' &&
    id === 'leads' &&
    action &&
    segs[3] === 'followup' &&
    method === 'POST'
  ) {

    const body = await request.json()

    const doc = {
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      title: body.title,
      dueAt: body.dueAt,
      status: 'pending',
      exportType: body.exportType || 'csv',
      format: 'csv',
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    }

    await db.collection('follow_ups').insertOne(doc);

    await db.collection('audit_logs').insertOne({
      id: crypto.randomUUID(),
      orgId,
      entity: 'followup',
      entityId: action,
      action: 'create',
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    await db.collection('lead_timeline').insertOne({
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      type: 'followup_created',
      payload: {
        title: body.title,
        dueAt: body.dueAt
      },
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    return json({
      success: true,
      followup: doc
    })
  }


  // PATCH /mobile/leads/:id/status
  if (
    root === 'mobile' &&
    id === 'leads' &&
    action &&
    segs[3] === 'status' &&
    method === 'PATCH'
  ) {

    const body = await request.json()

    const lead = await db.collection('leads').findOne({
      id: action,
      orgId
    })

    if (!lead) {
      return json({ error: 'Lead not found' }, { status: 404 })
    }

    await db.collection('leads').updateOne(
      { id: action, orgId },
      {
        $set: {
          status: body.status,
          updatedAt: new Date().toISOString()
        }
      }
    )

    await db.collection('lead_status_history').insertOne({
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      fromStatus: lead.status,
      toStatus: body.status,
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    await db.collection('audit_logs').insertOne({
      id: crypto.randomUUID(),
      orgId,
      entity: 'lead',
      entityId: action,
      action: 'update',
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    });

    return json({
      success: true
    })
  }


  // PATCH /mobile/leads/:id/assign
  if (
    root === 'mobile' &&
    id === 'leads' &&
    action &&
    segs[3] === 'assign' &&
    method === 'PATCH'
  ) {

    const body = await request.json()

    await db.collection('leads').updateOne(
      { id: action, orgId },
      {
        $set: {
          assignedTo: body.assignedTo,
          updatedAt: new Date().toISOString()
        }
      }
    )

    await db.collection('lead_assignments').insertOne({
      id: crypto.randomUUID(),
      orgId,
      leadId: action,
      assignedTo: body.assignedTo,
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    })

    return json({
      success: true
    })
  }


  // GET /mobile/sync
  if (
    root === 'mobile' &&
    id === 'sync' &&
    method === 'GET'
  ) {

    const url = new URL(request.url)
    const since = url.searchParams.get('since')

    const q = since
      ? {
          orgId,
          updatedAt: { $gt: since }
        }
      : { orgId }

    const leads = await db.collection('leads')
      .find(q)
      .project({ _id: 0 })
      .sort({ updatedAt: -1 })
      .limit(1000)
      .toArray()

    return json({
      success: true,
      count: leads.length,
      leads
    })
  }


  // GET /mobile/sync/changes
  if (
    root === 'mobile' &&
    id === 'sync' &&
    action === 'changes' &&
    method === 'GET'
  ) {

    const url = new URL(request.url)
    const since = url.searchParams.get('since')

    const q = since
      ? {
          orgId,
          createdAt: { $gt: since }
        }
      : { orgId }

    const changes = await db.collection('lead_timeline')
      .find(q)
      .project({ _id: 0 })
      .sort({ createdAt: -1 })
      .limit(500)
      .toArray()

    return json({
      success: true,
      count: changes.length,
      changes
    })
  }


  // GET /mobile/config
  if (
    root === 'mobile' &&
    id === 'config' &&
    method === 'GET'
  ) {

    return json({
      success: true,
      config: {
        pageSize: 20,
        syncBatchSize: 1000,
        timelineBatchSize: 500,
        version: '1.0.0'
      }
    })
  }


  // GET /mobile/bootstrap
  if (
    root === 'mobile' &&
    id === 'bootstrap' &&
    method === 'GET'
  ) {

    const [
      summary,
      leads,
      activities
    ] = await Promise.all([

      Promise.all([
        db.collection('leads').countDocuments({ orgId }),
        db.collection('follow_ups').countDocuments({
          orgId,
          status: 'pending',
        }),
        db.collection('lead_tasks').countDocuments({
          orgId,
          status: { $ne: 'completed' }
        })
      ]),

      db.collection('leads')
        .find({ orgId })
        .project({
          _id: 0,
          id: 1,
          name: 1,
          phone: 1,
          status: 1,
          assignedTo: 1,
          updatedAt: 1
        })
        .sort({ updatedAt: -1 })
        .limit(20)
        .toArray(),

      getRecentActivities(db, orgId, { limit: 20 }),
    ])

    return json({
      success: true,
      bootstrap: {
        stats: {
          totalLeads: summary[0],
          pendingFollowUps: summary[1],
          openTasks: summary[2]
        },
        recentLeads: leads,
        recentActivities: activities.activities,
      }
    })
  }


  // GET /mobile/leads/:id
  if (
    root === 'mobile' &&
    id === 'leads' &&
    action &&
    method === 'GET'
  ) {

    const leadId = action

    const [
      lead,
      timeline,
      tasks,
      followups,
      notes
    ] = await Promise.all([

      db.collection('leads').findOne(
        { id: leadId, orgId },
        { projection: { _id: 0 } }
      ),

      db.collection('lead_timeline')
        .find({ leadId, orgId })
        .project({ _id: 0 })
        .sort({ createdAt: -1 })
        .limit(50)
        .toArray(),

      db.collection('lead_tasks')
        .find({ leadId, orgId })
        .project({ _id: 0 })
        .sort({ createdAt: -1 })
        .toArray(),

      db.collection('follow_ups')
        .find({ leadId, orgId })
        .project({ _id: 0 })
        .sort({ createdAt: -1 })
        .toArray(),

      db.collection('lead_notes')
        .find({ leadId, orgId })
        .project({ _id: 0 })
        .sort({ createdAt: -1 })
        .toArray()
    ])

    if (!lead) {
      return json(
        { error: 'Lead not found' },
        { status: 404 }
      )
    }

    return json({
      lead,
      timeline,
      tasks,
      followups,
      notes
    })
  }


  // GET /mobile/summary
  if (root === 'mobile' && id === 'summary' && method === 'GET') {

    const [
      leadDashboard,
      followupDashboard,
      taskDashboard,
      recentActivities,
      latestLeads
    ] = await Promise.all([

      Promise.all([
        db.collection('leads').countDocuments({ orgId }),
        db.collection('lead_tasks').countDocuments({ orgId }),
        db.collection('lead_tasks').countDocuments({
          orgId,
          status: { $ne: 'completed' }
        }),
        db.collection('follow_ups').countDocuments({ orgId }),
        db.collection('lead_notes').countDocuments({ orgId }),
        db.collection('lead_status_history').countDocuments({ orgId })
      ]),

      Promise.all([
        db.collection('follow_ups').countDocuments({ orgId }),
        db.collection('follow_ups').countDocuments({
          orgId,
          status: 'pending',
      exportType: body.exportType || 'csv',
      format: 'csv'
        }),
        db.collection('follow_ups').countDocuments({
          orgId,
          status: 'completed'
        })
      ]),

      Promise.all([
        db.collection('lead_tasks').countDocuments({ orgId }),
        db.collection('lead_tasks').countDocuments({
          orgId,
          status: 'completed'
        }),
        db.collection('lead_tasks').countDocuments({
          orgId,
          status: { $ne: 'completed' }
        })
      ]),

      db.collection('lead_timeline')
        .find({ orgId })
        .project({ _id: 0 })
        .sort({ createdAt: -1 })
        .limit(10)
        .toArray(),

      db.collection('leads')
        .find({ orgId })
        .project({
          _id: 0,
          id: 1,
          name: 1,
          phone: 1,
          status: 1,
          assignedTo: 1,
          updatedAt: 1
        })
        .sort({ updatedAt: -1 })
        .limit(10)
        .toArray()
    ])

    return json({
      leadDashboard: {
        totalLeads: leadDashboard[0],
        totalTasks: leadDashboard[1],
        openTasks: leadDashboard[2],
        totalFollowUps: leadDashboard[3],
        totalNotes: leadDashboard[4],
        totalStatusChanges: leadDashboard[5]
      },
      followupDashboard: {
        totalFollowUps: followupDashboard[0],
        pendingFollowUps: followupDashboard[1],
        completedFollowUps: followupDashboard[2]
      },
      taskDashboard: {
        totalTasks: taskDashboard[0],
        completedTasks: taskDashboard[1],
        openTasks: taskDashboard[2]
      },
      recentActivities,
      latestLeads
    })
  }



  // GET /audit-logs
  if (
    root === 'audit-logs' &&
    method === 'GET'
  ) {

    const logs = await db.collection('audit_logs')
      .find({ orgId })
      .project({ _id: 0 })
      .sort({ createdAt: -1 })
      .limit(200)
      .toArray()

    return json({
      success: true,
      count: logs.length,
      logs
    })
  }

  // GET /audit-logs/:entity
  if (
    root === 'audit-logs' &&
    id &&
    method === 'GET'
  ) {

    const logs = await db.collection('audit_logs')
      .find({
        orgId,
        entityId: id
      })
      .project({ _id: 0 })
      .sort({ createdAt: -1 })
      .limit(200)
      .toArray()

    return json({
      success: true,
      count: logs.length,
      logs
    })
  }

  // GET /mobile/audit-logs
  if (
    root === 'mobile' &&
    id === 'audit-logs' &&
    method === 'GET'
  ) {

    const logs = await db.collection('audit_logs')
      .find({ orgId })
      .project({ _id: 0 })
      .sort({ createdAt: -1 })
      .limit(100)
      .toArray()

    return json({
      success: true,
      count: logs.length,
      logs
    })
  }


  // GET /analytics/conversion
  if (
    root === 'analytics' &&
    id === 'conversion' &&
    method === 'GET'
  ) {

    const total = await db.collection('leads')
      .countDocuments({ orgId })

    const won = await db.collection('leads')
      .countDocuments({
        orgId,
        status: 'Won'
      })

    return json({
      success: true,
      totalLeads: total,
      wonLeads: won,
      conversionRate: total
        ? Number(((won / total) * 100).toFixed(2))
        : 0
    })
  }

  // GET /analytics/sources
  if (
    root === 'analytics' &&
    id === 'sources' &&
    method === 'GET'
  ) {

    const data = await db.collection('leads')
      .aggregate([
        { $match: { orgId } },
        {
          $group: {
            _id: '$source',
            count: { $sum: 1 }
          }
        },
        { $sort: { count: -1 } }
      ])
      .toArray()

    return json({
      success: true,
      sources: data
    })
  }

  // GET /analytics/agents
  if (
    root === 'analytics' &&
    id === 'agents' &&
    method === 'GET'
  ) {

    const data = await db.collection('leads')
      .aggregate([
        { $match: { orgId } },
        {
          $group: {
            _id: '$assignedTo',
            leads: { $sum: 1 },
            won: {
              $sum: {
                $cond: [
                  { $eq: ['$status', 'Won'] },
                  1,
                  0
                ]
              }
            }
          }
        },
        { $sort: { leads: -1 } }
      ])
      .toArray()

    return json({
      success: true,
      agents: data
    })
  }

  // GET /analytics/funnel
  if (
    root === 'analytics' &&
    id === 'funnel' &&
    method === 'GET'
  ) {

    const data = await db.collection('leads')
      .aggregate([
        { $match: { orgId } },
        {
          $group: {
            _id: '$status',
            count: { $sum: 1 }
          }
        },
        { $sort: { count: -1 } }
      ])
      .toArray()

    return json({
      success: true,
      funnel: data
    })
  }

  // GET /mobile/analytics
  if (
    root === 'mobile' &&
    id === 'analytics' &&
    !action &&
    method === 'GET'
  ) {

    const [
      total,
      won,
      sources,
      funnel
    ] = await Promise.all([

      db.collection('leads')
        .countDocuments({ orgId }),

      db.collection('leads')
        .countDocuments({
          orgId,
          status: 'Won'
        }),

      db.collection('leads')
        .aggregate([
          { $match: { orgId } },
          {
            $group: {
              _id: '$source',
              count: { $sum: 1 }
            }
          }
        ])
        .toArray(),

      db.collection('leads')
        .aggregate([
          { $match: { orgId } },
          {
            $group: {
              _id: '$status',
              count: { $sum: 1 }
            }
          }
        ])
        .toArray()
    ])

    return json({
      success: true,
      analytics: {
        totalLeads: total,
        wonLeads: won,
        conversionRate: total
          ? Number(((won / total) * 100).toFixed(2))
          : 0,
        sources,
        funnel
      }
    })
  }


  // GET /analytics/trends
  if (
    root === 'analytics' &&
    id === 'trends' &&
    method === 'GET'
  ) {

    const leads = await db.collection('leads')
      .find({ orgId })
      .project({ _id: 0, createdAt: 1 })
      .toArray()

    const byDay = {}

    for (const lead of leads) {
      if (!lead.createdAt) continue
      const day = String(lead.createdAt).substring(0, 10)
      byDay[day] = (byDay[day] || 0) + 1
    }

    return json({
      success: true,
      trends: Object.entries(byDay)
        .sort((a, b) => a[0].localeCompare(b[0]))
        .map(([date, count]) => ({ date, count }))
    })
  }

  // GET /analytics/monthly
  if (
    root === 'analytics' &&
    id === 'monthly' &&
    method === 'GET'
  ) {

    const data = await db.collection('leads')
      .aggregate([
        { $match: { orgId } },
        {
          $group: {
            _id: { $substr: ['$createdAt', 0, 7] },
            count: { $sum: 1 }
          }
        },
        { $sort: { _id: 1 } }
      ])
      .toArray()

    return json({
      success: true,
      monthly: data
    })
  }

  // GET /mobile/analytics/trends
  if (
    root === 'mobile' &&
    id === 'analytics' &&
    action === 'trends' &&
    method === 'GET'
  ) {

    const leads = await db.collection('leads')
      .find({ orgId })
      .project({ _id: 0, createdAt: 1 })
      .toArray()

    const byDay = {}

    for (const lead of leads) {
      if (!lead.createdAt) continue
      const day = String(lead.createdAt).substring(0, 10)
      byDay[day] = (byDay[day] || 0) + 1
    }

    return json({
      success: true,
      trends: Object.entries(byDay)
        .sort((a, b) => a[0].localeCompare(b[0]))
        .map(([date, count]) => ({ date, count }))
    })
  }



  // GET /reports/exports
  if (
    root === 'reports' &&
    id === 'exports' &&
    method === 'GET'
  ) {

    const exportsList = await db.collection('report_exports')
      .find({ orgId })
      .project({ _id: 0 })
      .sort({ createdAt: -1 })
      .limit(100)
      .toArray()

    return json({
      success: true,
      count: exportsList.length,
      exports: exportsList
    })
  }

  // POST /reports/export
  if (
    root === 'reports' &&
    id === 'export' &&
    method === 'POST'
  ) {

    const body = await request.json()

    const exportType = body.exportType || 'csv'
    const reportType = body.type || 'leads'
    const ext = exportType === 'pdf' ? 'pdf' : exportType === 'xlsx' ? 'xlsx' : 'csv'

    const exportJob = {
      id: uuid(),
      orgId,
      type: reportType,
      filters: body.filters || {},
      status: 'pending',
      exportType,
      format: ext,
      createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true
    }

    await db.collection('report_exports').insertOne(exportJob)

    const fileName = `${reportType}_${Date.now()}.${ext}`

    await db.collection('report_exports').updateOne(
      { id: exportJob.id, orgId },
      { $set: { status: 'processing' } }
    )

    let exportResult

    if (exportType === 'pdf') {
      const branding = await getOrgBranding(db, orgId)
      let rows = []
      let title = 'Report'
      let headers = []
      let rowMapper = () => []

      if (reportType === 'leads') {
        rows = await db.collection('leads').find({ orgId }).project({ _id: 0 }).toArray()
        title = 'Leads Report'
        headers = ['Name', 'Phone', 'Status', 'Territory', 'Score']
        rowMapper = (l) => [l.name || '', l.phone || '', l.status || '', l.territory || '', String(l.score || 0)]
      } else if (reportType === 'revenue') {
        rows = await db.collection('revenue').find({ orgId }).project({ _id: 0 }).toArray()
        title = 'Revenue Report'
        headers = ['Client', 'Amount', 'Status', 'Source']
        rowMapper = (r) => [r.clientName || '', String(r.amount || 0), r.status || '', r.source || '']
      } else if (reportType === 'campaigns') {
        rows = await db.collection('campaigns').find({ orgId }).project({ _id: 0 }).toArray()
        title = 'Campaigns Report'
        headers = ['Name', 'Channel', 'Status', 'Scheduled']
        rowMapper = (c) => [c.name || '', c.channel || '', c.status || '', c.scheduledAt || '']
      } else if (reportType === 'customers') {
        rows = await db.collection('customers').find({ orgId }).project({ _id: 0 }).toArray()
        title = 'Customers Report'
        headers = ['Name', 'Company', 'Email', 'Status']
        rowMapper = (c) => [c.name || '', c.company || '', c.email || '', c.status || '']
      } else {
        return json({ error: 'Unsupported report type for PDF' }, { status: 400 })
      }

      exportResult = await generateReportPdf({ title, headers, rows, rowMapper, branding }, fileName)
    } else if (reportType === 'leads') {
      const leads = await db.collection('leads').find({ orgId }).project({ _id: 0 }).toArray()
      exportResult = exportType === 'xlsx'
        ? await generateLeadXlsx(leads, fileName)
        : await generateLeadCsv(leads, fileName)
    } else {
      return json({ error: 'CSV/XLSX exports support leads type only; use PDF for other types' }, { status: 400 })
    }

    await db.collection('report_exports').updateOne(
      {
        id: exportJob.id,
        orgId
      },
      {
        $set: {
          status: 'completed',
          fileUrl: `/storage/exports/${fileName}`,
          fileSize: exportResult.fileSize,
          totalRows: exportResult.totalRows,
          completedAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
        }
      }
    )

    return json({
      success: true,
      exportId: exportJob.id
    })
  }

  // GET /mobile/reports
  if (
    root === 'mobile' &&
    id === 'reports' &&
    method === 'GET'
  ) {

    const exportsList = await db.collection('report_exports')
      .find({ orgId })
      .project({ _id: 0 })
      .sort({ createdAt: -1 })
      .limit(20)
      .toArray()

    return json({
      success: true,
      exports: exportsList
    })
  }


  // =====================================================
  // PHASE 7C DOWNLOAD ENGINE
  // GET /reports/export/:id/download
  // =====================================================
  if (
    root === 'reports' &&
    id === 'export' &&
    action &&
    method === 'GET' &&
    request.url.endsWith('/download')
  ) {

    const exportJob = await db
      .collection('report_exports')
      .findOne({
        id: action,
        orgId
      })

    if (!exportJob) {
      return json(
        { error: 'Export not found' },
        { status: 404 }
      )
    }

    await db.collection('report_exports').updateOne(
      {
        id: action,
        orgId
      },
      {
        $inc: {
          downloadCount: 1
        },
        $set: {
          lastDownloadedAt: new Date().toISOString()
        }
      }
    )

    await db.collection('audit_logs').insertOne({
      id: uuid(),
      orgId,
      entityType: 'report_export',
      entityId: action,
      action: 'report_downloaded',
      createdAt: new Date().toISOString()
    })

    const fileName = exportJob.fileUrl?.split('/').pop()
    const file = readExportFile(fileName)
    if (file) {
      const isXlsx = fileName.endsWith('.xlsx')
      const isPdf = fileName.endsWith('.pdf')
      return new Response(file.buffer, {
        headers: {
          'Content-Type': isPdf
            ? 'application/pdf'
            : isXlsx
              ? 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
              : 'text/csv',
          'Content-Disposition': `attachment; filename="${file.fileName}"`,
        },
      })
    }

    return json({
      success: true,
      exportId: exportJob.id,
      fileUrl: exportJob.fileUrl || null,
      downloadCount: (exportJob.downloadCount || 0) + 1,
      mobileCompatible: true
    })
  }

  // =====================================================
  // GET /mobile/reports/:id/download
  // =====================================================
  if (
    root === 'mobile' &&
    id === 'reports' &&
    action &&
    method === 'GET' &&
    request.url.endsWith('/download')
  ) {

    const exportId = segs[2]

    const exportJob = await db
      .collection('report_exports')
      .findOne({
        id: exportId,
        orgId
      })

    if (!exportJob) {
      return json(
        { error: 'Export not found' },
        { status: 404 }
      )
    }

    await db.collection('report_exports').updateOne(
      {
        id: exportId,
        orgId
      },
      {
        $inc: {
          downloadCount: 1
        }
      }
    )

    const fileName = exportJob.fileUrl?.split('/').pop()
    const file = readExportFile(fileName)
    if (file) {
      const isXlsx = fileName?.endsWith('.xlsx')
      const isPdf = fileName?.endsWith('.pdf')
      return new Response(file.buffer, {
        headers: {
          'Content-Type': isPdf
            ? 'application/pdf'
            : isXlsx
              ? 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
              : 'text/csv',
          'Content-Disposition': `attachment; filename="${file.fileName}"`,
        },
      })
    }

    return json({
      success: true,
      exportId,
      fileUrl: exportJob.fileUrl || null,
      mobileCompatible: true
    })
  }


  // =====================================================
  // PHASE 7B REPORT GENERATION ENGINE
  // GET /reports/export/:id
  // =====================================================
  if (
    root === 'reports' &&
    id === 'export' &&
    action &&
    method === 'GET'
  ) {

    const exportJob = await db
      .collection('report_exports')
      .findOne({
        id: action,
        orgId
      })

    if (!exportJob) {
      return json(
        { error: 'Export not found' },
        { status: 404 }
      )
    }

    return json({
      success: true,
      export: exportJob,
      mobileCompatible: true
    })
  }

  // =====================================================
  // PATCH /reports/export/:id
  // status update endpoint
  // =====================================================
  if (
    root === 'reports' &&
    id === 'export' &&
    action &&
    method === 'PATCH'
  ) {

    const body = await request.json()

    const exportJob = await db
      .collection('report_exports')
      .findOne({
        id: action,
        orgId
      })

    if (!exportJob) {
      return json(
        { error: 'Export not found' },
        { status: 404 }
      )
    }

    await db.collection('report_exports').updateOne(
      {
        id: action,
        orgId
      },
      {
        $set: {
          status: body.status || exportJob.status,
          fileUrl: body.fileUrl || exportJob.fileUrl || null,
          fileSize: body.fileSize || exportJob.fileSize || 0,
          totalRows: body.totalRows || exportJob.totalRows || 0,
          completedAt: body.completedAt || null,
          updatedAt: new Date().toISOString()
        }
      }
    )

    await db.collection('audit_logs').insertOne({
      id: uuid(),
      orgId,
      entityType: 'report_export',
      entityId: action,
      action: 'export_status_updated',
      metadata: {
        status: body.status || exportJob.status
      },
      createdAt: new Date().toISOString()
    })

    return json({
      success: true,
      exportId: action
    })
  }


  // ─────────────────────────  KPIs (LeadEdge)  ─────────────────────────
  if (root === 'kpis' && method === 'GET') {
    const col = db.collection('leads')
    const url = new URL(request.url)
    const role = url.searchParams.get('role')
    const agentName = url.searchParams.get('agent')


    const q = { orgId, ...(role === 'agent' && agentName ? { assignedTo: agentName } : {}) }
    const leads = await col.find(q, { projection: { _id: 0 } }).toArray()
    const total = leads.length
    const qualified = leads.filter(l => ['Qualified', 'Proposal', 'Won'].includes(l.status)).length
    const won = leads.filter(l => l.status === 'Won').length
    const hot = leads.filter(l => l.label === 'Hot').length
    const conv = total ? Math.round((won / total) * 1000) / 10 : 0
    const avgScore = total ? Math.round(leads.reduce((s, l) => s + (l.score || 0), 0) / total) : 0

    const byTerr = {}; const byStatus = Object.fromEntries(STATUSES.map(s => [s, 0]))
    const bySource = {}; const byAgent = {}
    for (const l of leads) {
      byTerr[l.territory] = (byTerr[l.territory] || 0) + 1
      byStatus[l.status] = (byStatus[l.status] || 0) + 1
      bySource[l.source] = (bySource[l.source] || 0) + 1
      const k = l.assignedTo || 'Unassigned'
      byAgent[k] = byAgent[k] || { name: k, leads: 0, won: 0 }
      byAgent[k].leads += 1
      if (l.status === 'Won') byAgent[k].won += 1
    }
    const days = 14
    const trend = []
    for (let i = days - 1; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i); d.setHours(0, 0, 0, 0)
      const next = new Date(d); next.setDate(next.getDate() + 1)
      const same = leads.filter(l => {
        const t = new Date(l.createdAt).getTime()
        return t >= d.getTime() && t < next.getTime()
      })
      trend.push({
        date: d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }),
        leads: same.length, won: same.filter(l => l.status === 'Won').length,
      })
    }
    return json({
      total, qualified, won, hot, avgScore, conversion: conv,
      byTerritory: Object.entries(byTerr).map(([name, leads]) => ({ name, leads })),
      byStatus: Object.entries(byStatus).map(([name, value]) => ({ name, value })),
      bySource: Object.entries(bySource).map(([name, value]) => ({ name, value })),
      byAgent: Object.values(byAgent).map(a => ({ ...a, conversion: a.leads ? Math.round((a.won / a.leads) * 1000) / 10 : 0 })),
      trend,
    })
  }

  // ─────────────────────────  RETAILEDGE360 — PRODUCTS  ─────────────────────────
  if (root === 'products') {
    const col = db.collection('products')
    if (method === 'GET' && !id) {
      const list = await col.find({ orgId }, { projection: { _id: 0 } }).sort({ predictedShelfDays: 1 }).toArray()
      return json({ products: list })
    }
    if (method === 'POST' && !id) {
      const body = await request.json()
      if (!body?.name || !body?.sku) return err('name and sku are required')
      const pred = await predictShelfLife(body)
      const doc = {
        id: uuid(), orgId,
        name: body.name, sku: body.sku, category: (body.category || 'other').toLowerCase(),
        price: Number(body.price || 0), stock: Number(body.stock || 0),
        daysOnShelf: Number(body.daysOnShelf || 0),
        expiryDate: body.expiryDate || addDays(30), store: body.store || 'Default Store',
        ...pred,
        createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true, updatedAt: new Date().toISOString(),
      }
      await col.insertOne({ ...doc })
      delete doc._id
      return json({ product: doc }, { status: 201 })
    }
    if (method === 'POST' && id && action === 'repredict') {
      const p = await col.findOne({ id, orgId }, { projection: { _id: 0 } })
      if (!p) return err('Product not found', 404)
      const pred = await predictShelfLife(p)
      const r = await col.findOneAndUpdate({ id, orgId },
        { $set: { ...pred, updatedAt: new Date().toISOString() } },
        { returnDocument: 'after', projection: { _id: 0 } })
      return json({ product: r?.value || r })
    }
    if (method === 'DELETE' && id) {
      await col.deleteOne({ id, orgId })
      return json({ ok: true })
    }
  }

  // ─── RetailEdge KPIs ───
  if (root === 'retail-kpis' && method === 'GET') {
    const products = await db.collection('products').find({ orgId }, { projection: { _id: 0 } }).toArray()
    const total = products.length
    const highRisk = products.filter(p => p.risk === 'High').length
    const medRisk  = products.filter(p => p.risk === 'Medium').length
    const lowRisk  = products.filter(p => p.risk === 'Low').length
    const inventoryValue = products.reduce((s, p) => s + (p.price || 0) * (p.stock || 0), 0)
    const atRiskValue = products
      .filter(p => p.risk === 'High')
      .reduce((s, p) => s + (p.price || 0) * (p.stock || 0), 0)
    const byCategory = {}
    for (const p of products) {
      byCategory[p.category] = byCategory[p.category] || { name: p.category, count: 0, value: 0 }
      byCategory[p.category].count += 1
      byCategory[p.category].value += (p.price || 0) * (p.stock || 0)
    }
    return json({
      total, highRisk, medRisk, lowRisk,
      inventoryValue, atRiskValue,
      savedSoFar: Math.round(atRiskValue * 0.65), // RevenueShield AI projection
      byRisk: [
        { name: 'High', value: highRisk },
        { name: 'Medium', value: medRisk },
        { name: 'Low', value: lowRisk },
      ],
      byCategory: Object.values(byCategory),
    })
  }

  // ─────────────────────────  BILLING (RAZORPAY)  ─────────────────────────
  if (root === 'billing') {
    if (id === 'plans' && method === 'GET') return json({ plans: PLANS, configured: RZP_CONFIGURED })

    if (id === 'checkout' && method === 'POST') {
      const { planId } = await request.json()
      const plan = findCheckoutPlan(planId)
      if (!plan || plan.custom) return err('Invalid plan or custom — please contact sales')
      const rzp = getRazorpay()
      if (!rzp) {
        return json({ error: 'razorpay-not-configured', fallback: '/contact' }, { status: 503 })
      }
      const order = await rzp.orders.create({
        amount: plan.price * 100,
        currency: plan.currency,
        receipt: `r_${Date.now()}`,
        notes: { planId: plan.id, planCode: plan.planCode, product: plan.product, orgId },
      })
      await db.collection('payments').insertOne({
        id: uuid(), orgId, razorpay_order_id: order.id, amount: plan.price,
        plan: plan.id, planCode: plan.planCode, product: plan.product, status: 'created', createdAt: new Date(),
      })
      const branding = await getOrgBranding(db, orgId)
      const merchantName = branding.companyName || branding.legalName || ''
      return json({
        order,
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID,
        plan,
        planName: `${plan.productLabel || ''} ${plan.name}`.trim(),
        merchantName,
        companyName: merchantName,
      })
    }

    if (id === 'verify' && method === 'POST') {
      const body = await request.json()
      const ok = verifyCheckoutSignature({
        order_id: body.razorpay_order_id,
        payment_id: body.razorpay_payment_id,
        signature: body.razorpay_signature,
      })
      if (!ok) return err('Invalid signature', 400)
      const checkoutPlan = findCheckoutPlan(body.planId)
      const planCode = resolvePlanCode(body.planId || checkoutPlan?.planCode || 'lead-growth')
      const orgProducts = productsForPlan(checkoutPlan)
      await db.collection('payments').updateOne(
        { razorpay_order_id: body.razorpay_order_id },
        { $set: { status: 'paid', razorpay_payment_id: body.razorpay_payment_id, paidAt: new Date().toISOString(), planCode } }
      )
      await db.collection('orgs').updateOne(
        { id: orgId },
        {
          $set: {
            plan: planCode,
            products: orgProducts,
            activeProduct: orgProducts[0] || 'leadedge360',
            upgradedAt: new Date().toISOString(),
          },
        },
      )
      await db.collection('users').updateMany(
        { orgId },
        { $set: { products: orgProducts, activeProduct: orgProducts[0] || 'leadedge360', updatedAt: new Date().toISOString() } },
      )
      await db.collection('subscriptions').updateOne(
        { orgId, status: { $in: ['ACTIVE', 'TRIAL', 'PENDING'] } },
        { $set: { planCode, status: 'ACTIVE', updatedAt: new Date().toISOString() } },
        { upsert: false },
      )
      await emitPlatformEvent({
        db,
        orgId,
        type: PLATFORM_EVENTS.SUBSCRIPTION_CHANGED,
        entity: 'subscription',
        entityId: orgId,
        payload: { planCode, razorpayOrderId: body.razorpay_order_id },
        userId: tenant.user?.id || null,
        source: 'billing',
      })
      return json({ ok: true, planCode })
    }
  }

  // ─────────────────────────  WEBHOOKS  ─────────────────────────
  if (root === 'webhooks') {
    // Razorpay webhook (must use raw body for signature verification)
    if (id === 'razorpay' && method === 'POST') {
      const raw = await request.text()
      const sig = request.headers.get('x-razorpay-signature') || ''
      if (!verifyWebhookSignature(raw, sig)) return err('Invalid signature', 400)
      const evt = JSON.parse(raw)
      if (evt.event === 'order.paid' || evt.event === 'payment.captured') {
        const pay = evt.payload?.payment?.entity
        if (pay) {
          await db.collection('payments').updateOne(
            { razorpay_order_id: pay.order_id },
            { $set: { status: 'paid', razorpay_payment_id: pay.id, webhookAt: new Date().toISOString() } }
          )
        }
      }
      return json({ ok: true })
    }

    // Generic lead-ingest webhooks (WhatsApp / Facebook / Google) called by n8n
    if (['whatsapp', 'facebook', 'google'].includes(id) && method === 'POST') {
      if (!ingestWebhookAllowed(request)) return err('Unauthorized', 401)
      const body = await request.json()
      const targetOrgId = body.orgId
      const lead = {
        name: body.name || body.full_name || 'Unknown Lead',
        email: body.email || '',
        phone: body.phone || body.phone_number || body.wa_id || '',
        company: body.company || '',
        message: body.message || body.text || body.note || '',
        budget: Number(body.budget || 0),
        whatsapp: id === 'whatsapp' ? true : !!body.whatsapp,
        source: id,
        territory: body.territory || 'Bengaluru',
      }
      if (!lead.phone) return err('phone required', 400)
      const sc = await aiScore(lead)
      const agent = assignAgent(lead.territory)
      const attribution = await resolveLeadAttribution(db, targetOrgId, id, { ...body, ...lead }).catch(() => null)
      const doc = {
        id: uuid(), orgId: targetOrgId, ...lead,
        score: sc.score, label: sc.label, reasons: sc.reasons, engine: sc.engine,
        status: 'New', assignedTo: agent.name, assignedAgentId: agent.id,
        createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true, updatedAt: new Date().toISOString(),
        meta: { ...body, _source: id },
        ...(attribution ? {
          attribution,
          utm_source: attribution.utm_source,
          utm_medium: attribution.utm_medium,
          utm_campaign: attribution.utm_campaign,
        } : {}),
      }
      await db.collection('leads').insertOne({ ...doc })
      delete doc._id
      return json({ ok: true, lead: doc }, { status: 201 })
    }
  }

  // ─────────────────────────  CONTACT  ─────────────────────────
  if (root === 'contact' && method === 'POST') {
    const body = await request.json()
    if (!body?.email || !body?.message) return err('email and message are required')
    await db.collection('contact_requests').insertOne({
      id: uuid(), orgId, ...body, createdAt: new Date(),
      totalRows: 0,
      fileSize: 0,
      downloadCount: 0,
      mobileCompatible: true,
    })
    return json({ ok: true }, { status: 201 })
  }

  // ─────────────────────────  DEV  ─────────────────────────
  if (root === 'seed-reset' && method === 'POST') {
    await db.collection('leads').deleteMany({ orgId: DEMO_ORG_ID })
    await db.collection('products').deleteMany({ orgId: DEMO_ORG_ID })
    await seedDemoLeadsIfEmpty(db)
    await seedDemoProductsIfEmpty(db)
    return json({ ok: true, reseeded: true })
  }

  return err('Not found', 404)
}

export async function GET(req, { params })    { return route('GET', params.path || [], req) }
export async function POST(req, { params })   { return route('POST', params.path || [], req) }
export async function PATCH(req, { params })  { return route('PATCH', params.path || [], req) }
export async function PUT(req, { params })    { return route('PUT', params.path || [], req) }
export async function DELETE(req, { params }) { return route('DELETE', params.path || [], req) }
export async function OPTIONS() {
  return new NextResponse(null, { status: 204, headers: {
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'GET,POST,PATCH,PUT,DELETE,OPTIONS',
    'Access-Control-Allow-Headers': '*',
  }})
}

...
import { getBatch, updateBatch, deleteBatch } from '@/lib/retail/expiry/batches'

export async function GET(req, { params }) {
  try {
    const { orgId } = await guardExpiryRequest(req)
    const db = await getDb()
    const data = await getBatch(db, orgId, params.id)
...
export async function PUT(req, { params }) {
  try {
    ...
    const data = await updateBatch(db, orgId, user.id, params.id, body, meta)
    return NextResponse.json({ success: true, ...data })
  } catch (error) {
    return retailError(error)
  }
...
export async function DELETE(req, { params }) {
  try {
    ...
    const data = await deleteBatch(db, orgId, user.id, params.id, meta)
    return NextResponse.json({ success: true, ...data })
  } catch (error) {
    return retailError(error)
  }

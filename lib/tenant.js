...
import { isProductionMode } from './security/production'
...
export async function resolveTenant(request) {
  ...
  if (auth.startsWith('Bearer ')) {
    ...
    if (payload?.sub) {
      ...
      if (isDevAuthBypassEnabled()) {
        ...
        if (devUser) return { orgId: devUser.orgId, user: devUser, isDemo: false, authType: 'jwt-dev' }
      }
      try {
        const db = await getDb()
        ...
        // Mongo unavailable — dev JWT already handled above
      }
    }
  }
  ...
  try {
    ...
    if (!sessUser) {
      if (isProductionMode()) {
        return { orgId: null, user: null, isDemo: false, authType: 'anonymous' }
      }
...
/** Reject unauthenticated requests (except explicit demo read-only is not allowed for revenue). */
export async function requireAuthenticatedTenant(request) {

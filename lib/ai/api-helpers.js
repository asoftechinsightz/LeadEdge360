...
const AI_PLANS = ['STARTER', 'GROWTH', 'BUSINESS_GROWTH', 'PROFESSIONAL', 'PRO', 'ENTERPRISE']

export async function guardAiRequest(req, { feature = 'ai_assistant', permission = 'crm', soft = true } = {}) {
  const tenant = await requireAuthenticatedTenant(req)
  await requirePlan(tenant.orgId, AI_PLANS)

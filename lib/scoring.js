...
import { getAeoPrompt, renderPromptTemplate } from '@/lib/aeo/prompts'
...
/** AEO / AI Growth Engine prompts — separate from lead scoring. Same LLM path, config-driven templates. */
export async function runAeoPrompt(promptId, context = {}) {
  const tpl = getAeoPrompt(promptId)
  if (!tpl) return { ok: false, error: `Unknown prompt: ${promptId}`, engine: 'rules' }

  const key = process.env.EMERGENT_LLM_KEY
  if (!key) {
    return {
      ok: false,
      error: 'LLM key not configured — use rule-based recommendations',
      engine: 'rules-fallback',
    }
  }

  const userContent = renderPromptTemplate(tpl.userTemplate, context)
  const messages = []
  if (tpl.system) messages.push({ role: 'system', content: tpl.system })
  ...
  try {
    ...
    const r = await fetch(LLM_URL, {
      ...
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
      body: JSON.stringify({
        model: tpl.model || 'gpt-4o-mini',
        messages,
        temperature: tpl.temperature ?? 0.3,
        response_format: { type: 'json_object' },
      }),
      signal: ctrl.signal,
    })
    ...
    const txt = data?.choices?.[0]?.message?.content || '{}'
    const parsed = JSON.parse(txt)
    return { ok: true, data: parsed, engine: 'llm' }
  } catch (e) {
    return {
      ok: false,

...
import { signAccessToken, issueRefreshToken, rotateRefreshToken, revokeRefreshToken, verifyAccessToken, TOKEN_TTLS } from './jwt.js'
import { issueOtp, verifyOtp as verifyOtpCode } from './otp.js'
import { sendWhatsApp } from './whatsapp.js'
import { checkEntitlement } from './billing/plan-entitlements.js'
import { BRIDGE_ROOTS } from './request-actor.js'
import { mergeUserPreferences } from './aeo/preferences-merge.js'
import { writeAuditLog } from './billing/audit.js'

const json = (data, init = {}) => Response.json(data, init)
const err  = (code, message, status = 400, details) =>
  Response.json({ code, message, ...(details ? { details } : {}) }, { status })
...
//  AUTH
// =============================================================================
async function handleAuth(method, id, request, db) {
  // POST /auth/register
  if (id === 'register' && method === 'POST') {
    ...
    const exists = await db.collection('users').findOne({ $or: [{ email: body.email }, { phone: body.phone }] })
    if (exists) return err(exists.email === body.email ? 'AUTH_EMAIL_IN_USE' : 'AUTH_PHONE_IN_USE',
    ...
    const user = {
      id: uuid(), orgId,
      email: body.email, phone: body.phone, fullName: body.fullName,
      passwordHash: await hashPassword(body.password),
      role: 'admin', status: 'invited',
      emailVerified: false, phoneVerified: false,
      dpdpConsent: { ...(body.dpdpConsent || {}), acceptedAt: new Date().toISOString(), version: '1.0' },
      preferences: { notifications: { push: true, email: true, whatsapp: true } },
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    }
    await db.collection('users').insertOne(user)
    const o = await issueOtp({ destination: body.phone, purpose: 'signup', channel: 'sms' })

    return json({ userId: user.id, otpSent: true, ...(o.devOtp ? { devOtp: o.devOtp } : {}) }, { status: 201 })
  }
  ...
  if (id === 'verify-otp' && method === 'POST') {
    ...
    // mark user verified
    const user = await db.collection('users').findOne({ $or: [{ phone: destination }, { email: destination }] })
    if (!user) return err('USER_NOT_FOUND', 'No user for that destination', 404)
    const update = { updatedAt: new Date().toISOString() }
    if (purpose === 'signup' || purpose === 'verify') {
      if (user.phone === destination) update.phoneVerified = true
    ...
    if (purpose === 'login' || purpose === 'signup') {
      return json(await buildTokens(db, fresh, { ip: request.headers.get('x-forwarded-for') || '', ua: request.headers.get('user-agent') || '' }))
    }
    return json({ ok: true })
  }
  ...
  if (id === 'verify-email' && method === 'POST') {
    ...
    // For MVP we accept the user id as token; switch to signed token in production.
    const user = await db.collection('users').findOne({ id: token })
    if (!user) return err('VALIDATION_FAILED', 'Invalid token')
    await db.collection('users').updateOne({ id: user.id }, { $set: { emailVerified: true, status: 'active', updatedAt: new Date().toISOString() } })
    return json({ ok: true })
  }

  // POST /auth/login-otp  (send OTP to phone)
  if (id === 'login-otp' && method === 'POST') {
    ...
    const o = await issueOtp({ destination: phone, purpose: 'login', channel: 'sms' })
    return json({ ok: true, ...(o.devOtp ? { devOtp: o.devOtp } : {}) })
  }
  ...
  if (id === 'login-password' && method === 'POST') {
    ...
    if (!user || !user.passwordHash) return err('AUTH_INVALID_CREDENTIALS', 'Email or password is incorrect', 401)
    if (user.status === 'suspended') return err('AUTH_ACCOUNT_SUSPENDED', 'Account suspended', 403)
    const ok = await verifyPassword(body.password, user.passwordHash)
    if (!ok) return err('AUTH_INVALID_CREDENTIALS', 'Email or password is incorrect', 401)
    await db.collection('users').updateOne({ id: user.id }, { $set: { lastLoginAt: new Date().toISOString() } })
    return json(await buildTokens(db, user, { device: body.device || '', ip: request.headers.get('x-forwarded-for') || '', ua: request.headers.get('user-agent') || '' }))
  }
  ...
  if (id === 'forgot-password' && method === 'POST') {
    ...
    const o = await issueOtp({ destination, purpose: 'reset', channel: destination.includes('@') ? 'email' : 'sms' })
    return json({ ok: true, ...(o.devOtp ? { devOtp: o.devOtp } : {}) })
  }
...
// =============================================================================
async function handleUsers(method, id, request, db, user) {
  ...
  if (id === 'me' && method === 'PATCH') {
    const body = await request.json()
    const update = { updatedAt: new Date().toISOString() }
    if ('fullName' in body) update.fullName = body.fullName
    if ('picture' in body) update.picture = body.picture
    ...
    if ('preferences' in body) {
      ...
      const merged = mergeUserPreferences(current?.preferences, body.preferences)
      if (merged.error) {
        return err('VALIDATION_FAILED', merged.error, 400, merged.errors ? { errors: merged.errors } : undefined)
      }
    ...
    if (!result.matchedCount) return err('USER_NOT_FOUND', 'User not found', 404)

    if (body.preferences?.aeoProfile) {
      await writeAuditLog(db, {
        orgId: user.orgId,
        userId: user.id,
        action: 'aeo.profile.updated',
        entity: 'users',
        entityId: user.id,
        diff: { fields: Object.keys(body.preferences.aeoProfile || {}) },
        request,
      })
    }
  ...
  if (id === 'subscription' && method === 'GET') {
    ...
    const sub = await db.collection('subscriptions').findOne({ orgId: user.orgId, status: { $in: ['trialing', 'active'] } }, { projection: { _id: 0 } })
...
//  FOLLOW-UPS
// =============================================================================
async function handleFollowups(method, id, action, request, db, user) {
...
// =============================================================================
async function handleDashboard(method, id, request, db, user) {
...
//  WHATSAPP
// =============================================================================
async function handleWhatsapp(method, id, action, request, db, user) {
...
//  NOTIFICATIONS
// =============================================================================
async function handleNotifications(method, id, action, request, db, user) {
...
//  ADMIN
// =============================================================================
async function handleAdmin(method, segs, request, db, user) {
  ...
  if (root === 'users') {
    ...
    if (!id && method === 'POST') {
      ...
      const gate = await checkEntitlement(db, user.orgId, 'user.create')
      if (!gate.allowed) return Response.json(gate.body, { status: gate.status })
  ...
  if (root === 'roles' && method === 'GET') {
    return json({
      roles: [
        { name: 'admin',   description: 'Tenant administrator',  permissions: ['*'] },
        { name: 'manager', description: 'Sales manager',         permissions: ['leads.*', 'followups.*', 'dashboard.read', 'whatsapp.send', 'admin.users'] },
        { name: 'agent',   description: 'Sales agent',           permissions: ['leads.read', 'leads.write', 'followups.*', 'dashboard.read', 'whatsapp.send'] },
      ],
    })
  }
  ...
  if (root === 'product-access' && method === 'GET') {
    ...
    return json({
      access: [
        { productCode: 'leadedge360',  enabled: org?.leadEnabled ?? true },
...
// =============================================================================
//  SHARED DISPATCH (JWT + cookie bridge)
// =============================================================================
async function dispatchBridgedHandlers({ root, id, action, segs, method, request, db, user }) {
  if (root === 'users')         return handleUsers(method, id, request, db, user)
  if (root === 'followups')     return handleFollowups(method, id, action, request, db, user)
  if (root === 'dashboard')     return handleDashboard(method, id, request, db, user)
  if (root === 'whatsapp')      return handleWhatsapp(method, id, action, request, db, user)
  if (root === 'notifications') return handleNotifications(method, id, action, request, db, user)
  if (root === 'admin')         return handleAdmin(method, segs, request, db, user)
  return null
}
...
//  PUBLIC EXPORT
// =============================================================================
export { BRIDGE_ROOTS } from './request-actor.js'

/** Cookie bridge — same handlers as JWT when WEB_JWT_BRIDGE is enabled. */
export async function cookieBridgeRoute({ root, id, action, segs, method, request, user }) {
  const db = await getDb()
  return dispatchBridgedHandlers({ root, id, action, segs, method, request, db, user })
}

export async function mobileRoute({ root, id, action, segs, method, request }) {
  const db = await getDb()
  if (root === 'auth') return handleAuth(method, id, request, db)

  if (!BRIDGE_ROOTS.includes(root)) return null

  const a = await requireAuth(request)
  if (a.error) return a.error

  return dispatchBridgedHandlers({ root, id, action, segs, method, request, db, user: a.user })
}
